  ...
  computed: {
    configComponent() {
      ...
      if ( haveProviders.includes(this.provider) ) {
        return () => import(/* webpackChunkName: "machine-config" */ `@shell/machine-config/${ this.provider }`);
      }

      return () => import(/* webpackChunkName: "machine-config" */ `@shell/machine-config/generic`);
    },
